...
func TestCreateOne(t *testing.T) {
	...
	tests := []struct {
		name string
		rec  func(data *harness.Data) *record.PlayerCharacter
		err  bool
	}{
		{
			name: "Without ID",
	...
	for _, tc := range tests {
		...
		func() {
			...
			// repository
			r := h.Model.(*model.Model).PlayerCharacterRepository()
...
func TestGetOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "With ID",
			id: func(data *harness.Data) string {
				return data.PlayerCharacterRecs[0].ID
			},
			err: false,
		},
		{
...
func TestUpdateOne(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "With ID",
			rec: func(data *harness.Data) record.PlayerCharacter {
				return data.PlayerCharacterRecs[0]
			},
			err: false,
		},
		{
			name: "Without ID",
			rec: func(data *harness.Data) record.PlayerCharacter {
				rec := data.PlayerCharacterRecs[0]
				rec.ID = ""
				return rec
			},
			err: true,
